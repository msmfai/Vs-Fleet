...
check_tracked_absent 'not ready for (a )?(general )?public alpha yet|still blocked for public open-source release|No open-source license has been chosen yet|until the project license has been chosen and applied|license and artifact policy are resolved|UNLICENSED' \
  "public release-facing docs still describe unresolved alpha blockers" \
  README.md CONTRIBUTING.md LAUNCH.md PLAN.md NORTH_STAR.md docs/QUICKSTART.md docs/release/ALPHA_RELEASE_CHECKLIST.md docs/release/PUBLIC_ALPHA_DECISIONS.md docs/release/GITHUB_PUBLICATION_RUNBOOK.md

check_tracked_absent 'Private for now|macOS \+ Linux first-class together|scope is[[:space:]]+deliberately `local`[[:space:]]+\+[[:space:]]+\*existing\*[[:space:]]+`ssh`|\*\*In scope now\*\*:.*existing SSH' \
  "public root docs still contain stale private-license or broad-scope planning claims" \
  LAUNCH.md PLAN.md NORTH_STAR.md README.md
...
if [ ! -f docs/release/OWNER_DECISION_RECORD.md ]; then
  ...
  if ! scripts/check-owner-decisions.sh docs/release/OWNER_DECISION_RECORD.md; then
    ...
    if ! scripts/check-platform-support-decision.sh docs/release/OWNER_DECISION_RECORD.md .; then
      fail=1
    fi
