...
- **11.2 VS Code extension face (optional, recovers in-terminal signals).** A publishable extension using the **stable** shell-integration read stream (§8.1) to recover dropped OSC and feed the reporter; also offers in-editor jump/focus. Engine `^1.93.0`, **no proposed APIs**.
...
- **Phase 1.5 — Extra faces + QoL.** Optional VS Code extension face (read-stream recovery, §11.2), CLI, tray, tag grouping, focus modes, diff-summary, launch-run from sidebar.
