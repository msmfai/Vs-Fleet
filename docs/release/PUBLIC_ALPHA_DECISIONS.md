...
|---|---|---|---|
| License | `UNLICENSED` in Rust crates and VS Code packages; no `LICENSE` file. | Without a license, outsiders have no clear right to use, fork, package, or contribute. Changing later can be messy if external contributors appear. | Pick either MIT/Apache-2.0 dual license for permissive Rust ecosystem norms, or AGPL-3.0-only if you want strong network-copyleft. Do not publish while `UNLICENSED`; after choosing, run `scripts/apply-license-decision.sh` with reviewed legal license text, then run `scripts/check-license-decision.sh` to verify every manifest and lockfile agrees. |
