...
| Artifact policy | Tracked screenshots/eval outputs exist; some logs/JSON include personal paths and failed eval records. | Public repos should not leak local paths/process details or look like failing release evidence. | Keep a small curated screenshot set only if redacted and intentionally documented. Remove logs/RSS/raw eval outputs from public history before first GitHub publish if possible. |
